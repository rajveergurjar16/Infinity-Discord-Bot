import { listAutoPingConfigs } from './autoPingStore.js';
...
export async function handleAutoPingMemberJoin(member) {
  ...
  const settings = await listAutoPingConfigs(member.guild.id);
  await Promise.allSettled(settings.map(async (setting) => {
